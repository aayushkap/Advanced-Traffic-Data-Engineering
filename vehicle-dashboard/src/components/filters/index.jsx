...
export default function Filters() {
  ...
  const handleHistoryChange = (value) => {
    ...
    return {
      increment: historyOptions[Math.min(currentIndex + 1, historyOptions.length - 1)],
  ...
  return (
    <div className="filters-container">
      ...
      <CustomDropdown
        options= {[{label: "Per Minute", value: "Per Minute"}, {label: "Per Hour", value: "Per Hour"}, {label: "Per Day", value: "Per Day"}]}
        selected= {timeFilter}
      ...
      <label className="filter-container">
        <button className="history-button" disabled={history === historyOptions[0]}
         onClick={() => setHistory(handleHistoryChange(history).decrement)}>-</button>
        <input
          ...
          readOnly
        />
        <button className="history-button" disabled={history === historyOptions[historyOptions.length - 1]}
        onClick={() => setHistory(handleHistoryChange(history).increment)}>+</button>
        History
      </label>
